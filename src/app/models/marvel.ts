...
export namespace Convert {
    export function toMarvel(json: string): Marvel {
        return cast(JSON.parse(json), o("Marvel"));
    }

    export function marvelToJson(value: Marvel): string {
    ...
    function isValidObject(className: string, val: any): boolean {
        if (val === null || typeof val !== "object" || Array.isArray(val)) { return false; }
        const typeRep = typeMap[className];
